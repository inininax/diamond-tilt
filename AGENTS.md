...
- Test surfaces: headless 237/237 (`Scripts/run-tests.sh`, fast gate) · Unity EditMode 229/229 · Unity PlayMode smoke 6/6 (`-runTests -testPlatform PlayMode`).
- Scenes are registered in build settings by the bootstrapper; `Match.unity` runs a visible auto-match demo via `MatchAutoPlayer` that feeds the live economy (rewards/season/missions).
