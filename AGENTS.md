...
- Test surfaces: headless 236/236 (`Scripts/run-tests.sh`, fast gate) · Unity EditMode 229/229 · Unity PlayMode smoke 3/3 (`-runTests -testPlatform PlayMode`). Δ7 vs headless = dotnet-only adapter tests in `Tests/DotNet/Adapters/`.
- Scenes are registered in build settings by the bootstrapper; `Match.unity` runs a visible auto-match demo via `MatchAutoPlayer`.
