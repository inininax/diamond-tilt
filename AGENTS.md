...
- Test surfaces: headless 244/244 (`Scripts/run-tests.sh`, fast gate) · Unity EditMode 236/236 · Unity PlayMode 6/6 (`-runTests -testPlatform PlayMode`). Presentation: procedural 2.5D stadium + capsule characters + real-trajectory ball (`FieldPresenter`/`PlayerFigure`), broadcast camera.
- `Boot.unity` is the playable game (`MatchPlayController`: tap zone to pitch, SWING button to bat); `Match.unity` runs an auto-match demo. Both feed the live economy (rewards/season/missions).
